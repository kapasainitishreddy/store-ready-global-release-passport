...
const findingId = (rule, file, line) => {
  let hash = 0xcbf29ce484222325n;
  for (const byte of encoder.encode(`${rule}:${file}:${line}`)) hash = ((hash ^ BigInt(byte)) * 0x100000001b3n) & 0xffffffffffffffffn;
...
export function scanReleaseProofFiles(input, { project = 'repository', ref = 'local', scannedAt = new Date().toISOString(), inventoryPaths, externalCoverageGaps = [], profile = 'fixture' } = {}) {
  if (!Array.isArray(input) || input.length > MAX_FILES) throw new Error('Repository file limit exceeded');
  if (inventoryPaths !== undefined && (!Array.isArray(inventoryPaths) || inventoryPaths.length > 100000)) throw new Error('Repository inventory limit exceeded');
  if (!Array.isArray(externalCoverageGaps) || externalCoverageGaps.length > 20) throw new Error('Invalid coverage gaps');
  const files = new Map();
  let totalBytes = 0;
  for (const row of input) {
    ...
    const size = encoder.encode(row.content).byteLength;
  ...
  const knownPaths = new Set(inventoryPaths === undefined ? files.keys() : inventoryPaths.map(normalizePath));
  for (const file of files.keys()) knownPaths.add(file);

  const findings = [];
  const checks = [];
  const coverageGaps = externalCoverageGaps.map(item => ({ category: String(item.category || 'repository').slice(0, 80), status: 'Coverage unavailable', reason: String(item.reason || 'Coverage was not established.').slice(0, 500) }));
  ...
  const publicProfile = profile === 'public-github';
  const generic = publicProfile && !packageText;
  if (generic) coverageGaps.push({ category: 'profile', status: 'Coverage unavailable', reason: 'No root package.json was read. Node.js release checks and mobile platform requirements were not assessed.' });
  ...
  check('manifest', 'configuration', 'Project manifest', generic ? 'NOT_APPLICABLE' : packageJson ? 'PASS' : 'FAIL', generic ? 'Node.js manifest check is not applicable to this generic repository profile.' : packageJson ? 'package.json parsed successfully.' : 'A valid package.json is required by this web-project profile.');
  ...
  const sourceFiles = [...files.entries()].filter(([file]) => sourcePattern.test(file));
  let secretCount = 0;
  ...
  const sampledSourceFiles = [...knownPaths].filter(file => sourcePattern.test(file)).length > sourceFiles.length;
  check('secrets', 'security', 'Secret pattern scan', secretCount ? 'FAIL' : sampledSourceFiles ? 'WARNING' : 'PASS', secretCount ? `${secretCount} credential-like assignment(s) require review.` : sampledSourceFiles ? 'No match in sampled files; other source files were not read.' : 'No credential-like source assignments matched the built-in rule.');

  const scripts = packageJson?.scripts || {};
  const hasTestFiles = [...knownPaths].some(file => /(?:^|\/)(?:test|tests|__tests__)\/.*\.(?:mjs|cjs|js|ts|tsx|jsx)$|\.(?:test|spec)\.(?:mjs|cjs|js|ts|tsx|jsx)$/.test(file));
  const hasTestCommand = typeof scripts.test === 'string' && scripts.test.trim().length > 0;
  if (generic) check('tests', 'testing', 'Test configuration', 'NOT_APPLICABLE', 'Node.js test configuration was not evaluated for this repository.');
  else if (hasTestCommand && hasTestFiles) check('tests', 'testing', 'Test configuration', 'PASS', 'A test command and test source files are present. This static check does not execute project code.');
  else {
  ...
  check('build', 'build', 'Build configuration', generic ? 'NOT_APPLICABLE' : hasBuild ? 'PASS' : 'WARNING', generic ? 'Node.js build configuration was not evaluated for this repository.' : hasBuild ? 'A build script is present; this audit does not execute it.' : 'No build script was found.');
  if (!generic && !hasBuild) findings.push(makeFinding({ category: 'build', severity: 'medium', title: 'Build command is not configured', description: 'package.json has no build script.', file: 'package.json', rule: 'build-configuration', scanner: 'releaseproof-static', whyItMatters: 'The release artifact cannot be reproduced from a documented build command.', remediation: 'Add and document a production build command.' }));
  ...
  const hasLock = ['package-lock.json', 'pnpm-lock.yaml', 'yarn.lock'].some(file => knownPaths.has(file));
  check('dependencies', 'dependencies', 'Dependency inventory', generic ? 'NOT_APPLICABLE' : Object.keys(dependencies).length === 0 || hasLock ? 'PASS' : 'WARNING', generic ? 'This profile does not inventory non-Node.js dependencies.' : Object.keys(dependencies).length === 0 ? 'No third-party packages are declared.' : `${Object.keys(dependencies).length} direct dependency entries found${hasLock ? ' with a lockfile' : ' without a lockfile'}.`);
  if (!generic && Object.keys(dependencies).length && !hasLock) findings.push(makeFinding({ category: 'dependencies', severity: 'medium', title: 'Dependency lockfile is missing', description: 'Direct dependencies are declared without a recognized lockfile.', file: 'package.json', rule: 'dependency-lockfile', scanner: 'releaseproof-static', whyItMatters: 'Installs can resolve to different transitive versions over time.', remediation: 'Commit the package manager lockfile and keep it synchronized.' }));
  if (!generic && Object.keys(dependencies).length) coverageGaps.push({ category: 'dependencies', status: 'Coverage unavailable', reason: 'No dependency advisory database was queried by this static profile.' });

  const workflows = [...knownPaths].filter(file => /^\.github\/workflows\/[^/]+\.ya?ml$/.test(file));
  ...
  const readme = [...knownPaths].find(file => /^readme\.md$/i.test(file));
  const license = [...knownPaths].find(file => /^license(?:\.md|\.txt)?$/i.test(file));
  ...
  const deployConfig = ['vercel.json', 'netlify.toml', 'Dockerfile', 'render.yaml', 'fly.toml'].some(file => knownPaths.has(file)) || workflows.some(file => /^\.github\/workflows\/(?:pages|deploy|deployment)\.ya?ml$/i.test(file));
  check('deployment', 'deployment', 'Deployment configuration', generic ? 'NOT_APPLICABLE' : deployConfig ? 'PASS' : 'WARNING', generic ? 'Deployment configuration for this platform was not evaluated.' : deployConfig ? 'A supported deployment configuration file is present.' : 'No supported deployment configuration file was found.');
  if (!generic && !deployConfig) findings.push(makeFinding({ category: 'deployment', severity: 'medium', title: 'Deployment configuration is missing', description: 'No supported deployment configuration was found.', rule: 'deployment-config', scanner: 'releaseproof-static', whyItMatters: 'The intended production deployment cannot be checked from repository configuration.', remediation: 'Document the platform and add its production configuration.' }));

  const envFiles = [...knownPaths].filter(file => /^\.env(?:\..+)?$/.test(file) && !/^\.env\.(?:example|sample|template)$/.test(file));
  ...
  check('privacy', 'privacy', 'Privacy declaration review', publicProfile ? 'NOT_RUN' : 'NOT_APPLICABLE', publicProfile ? 'Privacy declarations were not assessed.' : 'This web fixture has no mobile privacy declarations; runtime data use is not assessed.', false);
  check('permissions', 'permissions', 'Platform permission review', publicProfile ? 'NOT_RUN' : 'NOT_APPLICABLE', publicProfile ? 'Platform permissions were not assessed.' : 'This web fixture contains no mobile permission manifest.', false);
  check('mobile-store', 'mobile', 'Mobile-store readiness', publicProfile ? 'NOT_RUN' : 'NOT_APPLICABLE', publicProfile ? 'Mobile-store rules were not assessed.' : 'This fixture is a Node.js web project.', false);
  ...
  return {
    ...
    profile: profile === 'public-github' ? 'Bounded public GitHub static profile' : 'Local static release profile',
    demo: profile !== 'public-github',
    project: String(project).slice(0, 160),
    ref: String(ref).slice(0, 120),
    scannedAt,
    decision: decision.decision,
    readinessScore: profile === 'public-github' ? null : score,
    summary: decision,
    checks,
    specialistJobs: statuses,
    findings,
    coverageGaps,
    limitations: ['Static file checks do not execute tests or builds and cannot establish runtime behavior.', 'The built-in secret rule can miss credentials and can produce false positives.', 'Dependency advisories are not queried by this profile.', ...(profile === 'public-github' ? ['Public repository intake samples a bounded subset of files and does not certify security, compliance, store readiness, or approval.'] : [])]
  };
}
